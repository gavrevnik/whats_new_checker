...
- The active product scope is movies. Keep generic content-type infrastructure extensible, but do not restore music, restaurants, or other content types without an explicit request.
...
- Do not invoke paid or external APIs merely for routine verification. Use mocks by default; run a real Codex, TMDB, or OMDb smoke test only when the integration itself changed and the check is necessary. A real smoke test must not persist recommendations or otherwise mutate the library.
...
- Reuse existing storage, TMDB, OMDb, modal, and card helpers instead of creating parallel implementations.
- Preserve the Russian/original naming pair for movies and people throughout storage, API responses, and UI.
